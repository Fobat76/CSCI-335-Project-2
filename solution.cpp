...
std::vector<File*> FileAVL::query(size_t min, size_t max) {
    ...
    //AVL Tree is empty thus no file are within interval size [min, max] or [max, min]
    if(this->size() == 0){
        return result;
    }
    //AVL Tree is empty thus there are fiels within interval size [min, max] or [max, min]
    else{
        //Checking if the given paramer is in ascending order 
        if(min < max){

        }
        //Checking if the given paramer is in descending order 
        else if(min > max ){
